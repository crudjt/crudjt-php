...
var_dump(CRUD_JT::read($token) == $expected_data);
var_dump(CRUD_JT::update($token, $ed_data) === true);
var_dump(CRUD_JT::read($token) == $expected_ed_data);
var_dump(CRUD_JT::delete($token) === true);
var_dump(CRUD_JT::read($token) === null);
...
for ($i = 0; $i < $ttl; $i++) {
    ...
    var_dump(CRUD_JT::read($token_with_ttl) == json_decode(json_encode($expected), true));
    $expected_ttl--;
    sleep(1);
}
var_dump(CRUD_JT::read($token_with_ttl) === null);
...
var_dump(CRUD_JT::read($token) === null);
var_dump(CRUD_JT::update($token, $data) === false);
var_dump(CRUD_JT::delete($token) === false);
var_dump(CRUD_JT::update($token, $data) === false);
var_dump(CRUD_JT::read($token) === null);
...
for ($i = 0; $i < $silence_read; $i++) {
    ...
    var_dump(CRUD_JT::read($token_with_silence_read) == json_decode(json_encode($expected), true));
    $expected_silence_read--;
}
var_dump(CRUD_JT::read($token_with_silence_read) === null);
...
for ($i = 0; $i < $silence_read; $i++) {
    ...
    var_dump(CRUD_JT::read($token_with_ttl_and_silence_read) == json_decode(json_encode($expected), true));
    $expected_ttl--;
    $expected_silence_read--;
    sleep(1);
}
var_dump(CRUD_JT::read($token_with_ttl_and_silence_read) === null);
...
for ($j = 0; $j < 10; $j++) {
    ...
    $start = microtime(true);
    for ($i = 0; $i < REQUESTS; $i++) {
        CRUD_JT::update($tokens[$i], $ed_data);
    }
    echo "Elapsed: " . (microtime(true) - $start) . " seconds" . PHP_EOL;
...
for ($i = 0; $i < REQUESTS; $i++) {
    $previous_tokens[] = CRUD_JT::create($data);
}
for ($i = 0; $i < REQUESTS; $i++) {
    CRUD_JT::create($data);
}
...
for ($i = 0; $i < LIMIT_ON_wY_FOR_CACHE; $i++) {
    ...
    for ($j = 0; $j < REQUESTS; $j++) {
        CRUD_JT::read($previous_tokens[$j]);
    }
    echo "Elapsed: " . (microtime(true) - $start) . " seconds" . PHP_EOL;
}
